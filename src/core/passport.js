          ...
          if (req.user) {
            ...
            if (result.rowCount) {
              ...
              if (result.rowCount) {
                ...
                await query(
                  `INSERT INTO user_claim (user_id, type, value) VALUES($1, 'urn:facebook:access_token', $2);`,
                  userId,
                  accessToken,
                );
                await query(
                  `INSERT INTO user_profile (user_id, display_name, gender, picture) VALUES ($1, $2, $3, $4);`,
                  userId,
                  profile.displayName,
                  profile._json.gender,
                  `https://graph.facebook.com/${profile.id}/picture?type=large`,
